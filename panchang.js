...
class Panchang {
   ...
   calculate(dateInput, options = {}) {
      ...
      const getElements = (j) => {
         ...
         return { nakIdx, tithiIdx, yogaIdx, karanaIdx, ayan, lMoonNirayana, lSunNirayana };
      };
      ...
      // Kerala Panchangam: Sunrise-based day boundary
      // If Sankranti occurs before sunrise → that day is Day 1
      // If Sankranti occurs after sunrise → next day is Day 1
      const getSankrantiCivilJD = (s_jd) => {
         ...
         let sunriseMs = sunTimes.sunriseDate.getTime();
         let sankrantiMs = (s_jd - 2440587.5) * 86400000;
         let cjd = mdy2julian(dObj.getMonth() + 1, dObj.getDate(), dObj.getFullYear());
         // If sankranti is after sunrise, Day 1 shifts to next day
         if (sankrantiMs > sunriseMs) cjd += 1;
         return cjd;
      };
      ...
      */
      ...
      // Vishu - Medam 1 (First day of Malayalam New Year)
      // malMonth 0 = Medam
      if (malMonth === 0 && malDay === 1) {
      ...
      // Ekadashi/Amavasya/Pournami -> Based on Tithi at Sunrise
      // Pradosham -> Based on Tithi at Sunset
      if (sunTimes.sunriseDate && sunTimes.sunsetDate) {
         const sunriseJD = (sunTimes.sunriseDate.getTime() / 86400000.0) + 2440587.5;
         const sunsetJD = (sunTimes.sunsetDate.getTime() / 86400000.0) + 2440587.5;

         const sunriseTithi = getElements(sunriseJD).tithiIdx;
         const sunsetTithi = getElements(sunsetJD).tithiIdx;

         // Ekadashi (Sunrise)
         if (sunriseTithi === 10 || sunriseTithi === 25) specialEvents.push("ഏകാദശി");

         // Amavasya (Sunrise)
         if (sunriseTithi === 29) specialEvents.push("അമാവാസി");

         // Pournami (Sunrise)
         if (sunriseTithi === 14) specialEvents.push("പൗർണ്ണമി");

         // Pradosham (Sunset)
         if (sunsetTithi === 12 || sunsetTithi === 27) specialEvents.push("പ്രദോഷം");
      }

      // Calculate additional astronomical features
      
      // Moon phase calculation
      const moonPhase = this.calculateMoonPhase(jd);
      
      // Moonrise/moonset times
      const moonTimes = this.calculateMoonTimes(d, lat, lon);
      
      // Planetary positions (basic)
      const planetaryPositions = this.calculateBasicPlanetaryPositions(jd);
      
      // Panchak calculation
      const panchak = this.calculatePanchak(tithiData.tithiIdx, d);
      
      // Hora calculation
      const hora = this.calculateHora(d, lat, lon);
      
      // Nakshatra pada calculation
      const nakshatraDetails = this.calculateNakshatraPada(currentData.lMoonNirayana, currentData.nakIdx);
      
      // Hindu calendar details
      const hinduCalendar = this.calculateHinduCalendar(year, malMonth, malDay);
      
      // Shaka Samvat calculation
      const shakaSamvat = this.calculateShakaSamvat(year);
      
      // Vikram Samvat calculation
      const vikramSamvat = this.calculateVikramSamvat(year);
      
      // Tithi duration calculation
      const tithiDuration = this.calculateTithiDuration(jd);
      
      // Rashi details
      const rashiDetails = this.calculateRashiDetails(nirSun, rashiIdx);
      ...
      return {
         ...
         Nakshatra: {
            ...
            pada_start: nakshatraDetails.start,
            pada_end: nakshatraDetails.end,
            progress: nakshatraDetails.progress
         },
         DayNakshatras: dayNakshatras,
         Tithi: {
            name: TITH[tithiData.tithiIdx],
            end: tithiEndDate,
            duration: tithiDuration
         },
         Yoga: {
    ...
    * @returns {Object} Moon phase details
    */
   calculateMoonPhase(jd) {
      const sunLong = Astronomy.sun(jd);
      const moonLong = Astronomy.moon(jd);
      ...
      if (elongation < 6) {
         phaseName = 'New Moon';
      } else if (elongation < 84) {
         phaseName = 'Waxing Crescent';
      } else if (elongation < 96) {
         phaseName = 'First Quarter';
      } else if (elongation < 174) {
         phaseName = 'Waxing Gibbous';
      } else if (elongation < 186) {
         phaseName = 'Full Moon';
      } else if (elongation < 264) {
         phaseName = 'Waning Gibbous';
      } else if (elongation < 276) {
         phaseName = 'Last Quarter';
      } else if (elongation < 354) {
      ...
      illumination = Math.abs(Math.cos(elongation * d2r)) * 100;
    ...
    */
   calculateMoonTimes(date, lat, lon) {
      // This is a simplified calculation
      // For more accurate calculations, we'd need to implement complex algorithms
      const year = date.getFullYear();
      const month = date.getMonth() + 1;
      const day = date.getDate();
      
      const jd = mdy2julian(month, day, year);
      
      // Calculate moon position
      const moonLong = Astronomy.moon(jd);
      const sunLong = Astronomy.sun(jd);
      
      // Approximate moonrise/set based on position relative to sun
      // This is a simplified algorithm - real calculations are much more complex
      const sunTimes = Astronomy.getSunriseSunset(date, lat, lon);
      
      // Moon rises approximately 50 minutes later each day
      // Calculate based on elongation from sun
      let elongation = moonLong - sunLong;
      if (elongation < 0) elongation += 360;
      
      // Calculate approximate time offset
      const hourOffset = (elongation / 360) * 24;
      
      // Create approximate moon times
      const baseTime = sunTimes.sunriseDate ? sunTimes.sunriseDate.getTime() : 
                       new Date(year, month - 1, day, 6, 0, 0).getTime();
      
      const moonriseTime = new Date(baseTime + (hourOffset * 3600000) - (6 * 3600000));
      const moonsetTime = new Date(baseTime + (hourOffset * 3600000) + (6 * 3600000));
      
      return {
         moonrise: moonriseTime.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true }),
         moonset: moonsetTime.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true }),
         moonriseDate: moonriseTime,
         moonsetDate: moonsetTime
      };
   }
    ...
    * @param {number} jd - Julian day
    * @returns {Object} Basic planetary positions
    */
   calculateBasicPlanetaryPositions(jd) {
      // This is a simplified implementation for demonstration
      // Real planetary calculations are extremely complex
      const sunPos = Astronomy.sun(jd);
      const moonPos = Astronomy.moon(jd);
      
      // Calculate approximate positions for other planets
      // These are highly simplified for this implementation
      const t = (jd - 2451545.0) / 36525; // Julian centuries from J2000.0
      
      // Mercury (rough approximation)
      const mercuryLong = fix360(252.250906 + 149472.51287 * t);
      
      // Venus (rough approximation)
      const venusLong = fix360(181.979801 + 58517.80309 * t);
      
      // Mars (rough approximation)
      const marsLong = fix360(355.433 + 19140.259 * t);
      
      // Jupiter (rough approximation)
      const jupiterLong = fix360(34.35 + 3034.721 * t);
      
      // Saturn (rough approximation)
      const saturnLong = fix360(50.08 + 1222.169 * t);
      
      return {
         Sun: { longitude: sunPos, name: 'Sun' },
         Moon: { longitude: moonPos, name: 'Moon' },
         Mercury: { longitude: mercuryLong, name: 'Mercury' },
         Venus: { longitude: venusLong, name: 'Venus' },
         Mars: { longitude: marsLong, name: 'Mars' },
         Jupiter: { longitude: jupiterLong, name: 'Jupiter' },
         Saturn: { longitude: saturnLong, name: 'Saturn' }
      };
   }

   /**
    * Calculate Panchak (five inauspicious periods)
    * @param {number} tithiIdx - Tithi index
    * @param {Date} date - Date object
    * @returns {Object} Panchak details
    */
   calculatePanchak(tithiIdx, date) {
      // Panchak occurs during the last 7 tithis of Krishna Paksha (waning phase)
      // These are: 23 (Krishna Ashtami), 24 (Krishna Navami), 25 (Dashami), 
      // 26 (Ekadashi), 27 (Dwadashi), 28 (Trayodashi), 29 (Chaturdashi)
      
      const panchakTithis = [23, 24, 25, 26, 27, 28, 29];
      const isPanchak = panchakTithis.includes(tithiIdx);
      
      // Determine specific Panchak type
      let panchakType = '';
      if (isPanchak) {
         if (tithiIdx === 23) panchakType = 'Raj Panchak';
         else if (tithiIdx === 24) panchakType = 'Chor Panchak';
         else if (tithiIdx === 25) panchakType = 'Agnyaastra Panchak';
         else if (tithiIdx === 26) panchakType = 'Sarpa Panchak';
         else if (tithiIdx === 27) panchakType = 'Kal Panchak';
         else panchakType = 'Panchak';
      }
      
      // Also check for specific dangerous times within Panchak
      const dayOfWeek = date.getDay(); // 0 = Sunday, 1 = Monday, etc.
      const dangerDays = ['Tuesday', 'Saturday', 'Sunday'];
      const isDangerousDay = [2, 6, 0].includes(dayOfWeek);
      
      return {
         isPanchak: isPanchak,
         type: panchakType,
         isDangerousDay: isPanchak && isDangerousDay,
         dangerousDayName: isDangerousDay ? dangerDays[[2, 6, 0].indexOf(dayOfWeek)] : null
      };
   }
    ...
    * @param {number} lat - Latitude
    * @param {number} lon - Longitude
    * @returns {Object} Hora details
    */
   calculateHora(date, lat, lon) {
      const sunTimes = Astronomy.getSunriseSunset(date, lat, lon);
      if (!sunTimes.sunriseDate || !sunTimes.sunsetDate) {
         return { error: "Unable to calculate sunrise/sunset times" };
      }
      
      const sunrise = sunTimes.sunriseDate;
      const sunset = sunTimes.sunsetDate;
      
      // Calculate day length and hora duration
      const dayLength = sunset.getTime() - sunrise.getTime();
      const dayHoraDuration = dayLength / 12; // 12 day horas
      
      const nightLength = 24 * 3600000 - dayLength; // Milliseconds in a day minus day length
      const nightHoraDuration = nightLength / 12; // 12 night horas
      
      // Planetary order for Horas: Sun, Venus, Mercury, Moon, Saturn, Jupiter, Mars
      const planets = ['Sun', 'Venus', 'Mercury', 'Moon', 'Saturn', 'Jupiter', 'Mars'];
      
      // Lord of the day (based on weekday)
      const dayLord = planets[date.getDay()];
      
      // Calculate current Hora
      const currentTime = date.getTime();
      let currentHora = '';
      let currentHoraStart = null;
      let currentHoraEnd = null;
      let currentHoraPlanet = '';
      
      if (currentTime >= sunrise.getTime() && currentTime <= sunset.getTime()) {
         // Day time
         const elapsedDayTime = currentTime - sunrise.getTime();
         const horaIndex = Math.floor(elapsedDayTime / dayHoraDuration);
         
         // Calculate which planet rules this hora
         // Day starts with the lord of the day, then follows the planetary order
         const planetIndex = (date.getDay() + horaIndex) % 7;
         currentHoraPlanet = planets[planetIndex];
         currentHoraStart = new Date(sunrise.getTime() + (horaIndex * dayHoraDuration));
         currentHoraEnd = new Date(sunrise.getTime() + ((horaIndex + 1) * dayHoraDuration));
      } else {
         // Night time - starts after sunset and continues to next sunrise
         const nextSunrise = new Date(sunrise);
         nextSunrise.setDate(nextSunrise.getDate() + 1);
         
         let nightStart;
         if (currentTime > sunset.getTime()) {
             nightStart = sunset.getTime();
         } else {
             // Before sunrise, so previous night
         ...
         const elapsedNightTime = currentTime - nightStart;
         const horaIndex = Math.floor(elapsedNightTime / nightHoraDuration);
         
         // Night starts with the planet that follows the lord of the day
         const planetIndex = (date.getDay() + 1 + horaIndex) % 7;
         currentHoraPlanet = planets[planetIndex];
         currentHoraStart = new Date(nightStart + (horaIndex * nightHoraDuration));
         currentHoraEnd = new Date(nightStart + ((horaIndex + 1) * nightHoraDuration));
      }
      
      return {
         dayLord: dayLord,
         currentHora: currentHoraPlanet,
         currentHoraStart: currentHoraStart,
         currentHoraEnd: currentHoraEnd,
         dayHoraDuration: dayHoraDuration / 60000, // in minutes
         nightHoraDuration: nightHoraDuration / 60000, // in minutes
         planets: planets
      };
   }
    ...
    * @param {number} nakshatraIndex - Nakshatra index
    * @returns {Object} Pada details
    */
   calculateNakshatraPada(moonLongitude, nakshatraIndex) {
      ...
      const moonPositionInNakshatra = moonLongitude - nakshatraStart;
      
      // Calculate which pada the moon is in
      const padaLength = (360 / 27) / 4; // 3°20' in decimal degrees
      const padaNumber = Math.floor(moonPositionInNakshatra / padaLength) + 1;
      
      // Calculate pada start and end points
      const padaStart = nakshatraStart + (padaNumber - 1) * padaLength;
      const padaEnd = nakshatraStart + padaNumber * padaLength;
      
      // Calculate the percentage of completion in the pada
      const padaProgress = ((moonPositionInNakshatra % padaLength) / padaLength) * 100;
      
      return {
         pada: padaNumber,
         start: padaStart,
         end: padaEnd,
         progress: parseFloat(padaProgress.toFixed(2)),
         nakshatraIndex: nakshatraIndex
      };
   }

   /**
    * Calculate Hindu calendar details
    * @param {number} gregorianYear - Gregorian year
    * @param {number} malayalamMonth - Malayalam month index
    * @param {number} malayalamDay - Malayalam day
    * @returns {Object} Hindu calendar details
    */
   calculateHinduCalendar(gregorianYear, malayalamMonth, malayalamDay) {
      // Map Malayalam months to Hindu calendar months
      ...
      // For Malayalam calendar, map to corresponding Hindu month
      // This is a simplified mapping
      let hinduMonthIndex = malayalamMonth; // Same as rashi
      let hinduMonthName = monthNames[hinduMonthIndex];
      
      // Calculate lunar month (Shukla paksha or Krishna paksha)
      let paksha = malayalamDay <= 15 ? "Shukla" : "Krishna";
      let tithiInPaksha = paksha === "Shukla" ? malayalamDay : malayalamDay - 15;
      if (tithiInPaksha === 0) {
          tithiInPaksha = 15; // Last day of Krishna paksha
          paksha = "Krishna";
      }
      
      return {
         month: hinduMonthName,
         monthIndex: hinduMonthIndex,
         paksha: paksha,
         tithiInPaksha: tithiInPaksha,
         year: gregorianYear - 78 // Simplified - not accurate for all cases
      };
   }

   /**
    * Calculate Shaka Samvat
    * @param {number} gregorianYear - Gregorian year
    * @returns {number} Shaka Samvat year
    */
   calculateShakaSamvat(gregorianYear) {
      // Shaka Samvat = Gregorian Year - 78
      // But it changes around March-April (Chaitra month)
    ...
    * @param {number} gregorianYear - Gregorian year
    * @returns {number} Vikram Samvat year
    */
   calculateVikramSamvat(gregorianYear) {
   ...
   /**
    * Calculate Tithi duration
    * @param {number} jd - Julian day
    * @returns {Object} Tithi duration details
    */
   calculateTithiDuration(jd) {
      // Calculate when the current tithi ends and the next begins
      // This is a simplified calculation
      const tithiStartJD = jd;
      
      // Calculate moon and sun positions
      let moonLong = Astronomy.moon(tithiStartJD);
      let sunLong = Astronomy.sun(tithiStartJD);
      
      // Calculate current tithi
      let diff = moonLong - sunLong;
      if (diff < 0) diff += 360;
      const currentTithi = Math.floor(diff / 12);
      
      // Find when the next tithi begins (when diff reaches next multiple of 12)
      const targetDiff = (currentTithi + 1) * 12;
      
      // Iteratively find when the next tithi starts
      let checkJD = tithiStartJD;
      let iterations = 0;
      const maxIterations = 100; // Prevent infinite loop
      
      while (iterations < maxIterations) {
         let checkMoon = Astronomy.moon(checkJD);
         let checkSun = Astronomy.sun(checkJD);
         let checkDiff = checkMoon - checkSun;
         if (checkDiff < 0) checkDiff += 360;
         
         if (checkDiff >= targetDiff) {
            break;
         }
         checkJD += 0.1; // Increment by 0.1 days (~2.4 hours)
         iterations++;
      }
      
      const durationDays = checkJD - tithiStartJD;
      
      return {
         startJD: tithiStartJD,
         endJD: checkJD,
         durationHours: parseFloat((durationDays * 24).toFixed(2)),
         durationMinutes: parseFloat((durationDays * 24 * 60).toFixed(2))
      };
   }

   /**
    * Calculate Rashi details
    * @param {number} sunLongitude - Sun's longitude
    * @param {number} rashiIndex - Rashi index
    * @returns {Object} Rashi details
    */
   calculateRashiDetails(sunLongitude, rashiIndex) {
      // Define rashi names in Malayalam
      const rashiNames = ZN; // Use the ZN constant defined at the top
      
      // Calculate how far into the rashi the sun is
      const rashiStart = rashiIndex * 30;
      const sunPositionInRashi = sunLongitude - rashiStart;
      const rashiProgress = (sunPositionInRashi / 30) * 100;
      
      // Determine the sign quality (fire, earth, air, water)
      let element;
      if ([0, 3, 6, 9].includes(rashiIndex)) element = 'Fire'; // Mesh, Kanya, Thula, Dhanu
      else if ([1, 4, 7, 10].includes(rashiIndex)) element = 'Earth'; // Rishaba, Kataka, Kumbha, Makara
      else if ([2, 5, 8, 11].includes(rashiIndex)) element = 'Air'; // Mithuna, Simha, Kanya, Meena
      else element = 'Water'; // Other signs
      
      // Determine mobility (cardinal, fixed, mutable)
      let quality;
      if ([0, 3, 6, 9].includes(rashiIndex)) quality = 'Cardinal'; // Mesha, Kanya, Thula, Dhanu
      else if ([1, 4, 7, 10].includes(rashiIndex)) quality = 'Fixed'; // Vrishabha, Karka, Kumbha, Makara
      else quality = 'Mutable'; // Mithuna, Simha, Kanya, Meena
      
      // Note: The above classification is not entirely accurate for Indian astrology
      // Let's use the correct classification:
      if ([0, 4, 8].includes(rashiIndex)) quality = 'Cardinal'; // Mesha, Kanya, Dhanu (Chara)
      else if ([1, 5, 9].includes(rashiIndex)) quality = 'Fixed'; // Vrishabha, Karka, Makara (Sthira)
      else quality = 'Dual'; // Mithuna, Simha, Kanya, Thula, Kumbha, Meena (Dvisvabhava)
      ...
      return {
         ...
         progress: parseFloat(rashiProgress.toFixed(2)),
         element: element,
         quality: quality
      };
   }
   ...
   getMalayalamDateFast(d, lat, lon) {
      ...
      // 3. Determine Civil Day of Sankranti (Kerala: sunrise-based)
      const getSankrantiCivilJD = (s_jd) => {
